...
spec:
  ...
  - name: node-joiner
    ...
    - name: assets
      mountPath: /assets
    command: ["/bin/sh", "-c", "cp /config/nodes-config.yaml /assets; HOME=/assets node-joiner add-nodes --dir=/assets --log-level=debug; sleep 3600"]    
  volumes:
  - name: nodes-config
    configMap: 
      name: nodes-config
...
if [ "$res" = 0 ]; then
  ...
  oc cp -n "${namespace}" --retries=5 node-joiner:/assets/node.x86_64.iso node.x86_64.iso
else
